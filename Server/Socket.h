 ...
 * Devuelve el numero de bytes escritos o -1 si ha habido error. */
 ...
 * Se le pasa una estructura sockaddr que nos devolvera rellena con los datos del que nos
 * ha enviado el mensaje, de forma que podamos responderle.
 * Se le pasa el tamaño de la estructura Remoto. En la misma variable nos devolvera el
 ...
 * Devuelve el numero de bytes enviados o -1 si ha habido algun error.
 */
int Escribe_Socket_Udp (SOCKET fd, struct sockaddr *Remoto, 
    int Longitud_Remoto, char *Datos, int Longitud);

/**
 * Rellena una estructura sockaddr_in con los datos que se le pasan. Esta estructura es
 * util para el envio o recepcion de mensajes por sockets Udp o para abrir conexiones.
 * Se le pasa el host. Puede ser NULL (para abrir socket servidor Udp o para recepcion de
 * mensajes de cualquier host).
 * Se le pasa el servicio. Puede ser NULL (para abrir socket cliente Udp).
 * Se le pasa una estructura sockaddr_in que devolvera rellena.
 * Se le pasa una Longitud. Debe contener el tamaño de la estructura sockaddr_in y
 * devolvera el tamaño de la estructura una vez rellena.
